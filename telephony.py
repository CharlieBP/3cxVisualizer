            ...
            for key, filename in required_files.items():
                if filename in zip_base_filenames:
                    ...
                    if actual_zip_path:
                        ...
                        except Exception as e: st.error(f"Kon {filename} niet lezen uit ZIP: {e}"); all_files_found = False
                else:
...
def parse_destination(dest_string):
    ...
    if match_id_name: return "ExtensionNumber", match_id_name.group(1)
...
def get_node_label_and_style(identifier, type_hint, all_data):
    """Genereert label en bepaalt stijl, nu inclusief Q/RG tijden."""
    ...
    if pd.isna(identifier) or identifier == "": label = "Niet geconfigureerd"; node_type="ConfigError"
        ...
        # Check Queues
        if not queues_df.empty and (type_hint == "Queue" or type_hint == "ExtensionNumber" or type_hint == "UnknownType"):
            ...
            if not queue.empty:
                ...
                # Haal tijden op
                ring_time = queue_info.get('Ring time (s)', None)
                max_wait = queue_info.get('Max queue wait time (s)', None)
                ring_time_str = f"{int(ring_time)}s" if pd.notna(ring_time) and isinstance(ring_time,(int,float)) else "N/A"
                max_wait_str = f"{int(max_wait)}s" if pd.notna(max_wait) and isinstance(max_wait,(int,float)) else "N/A"
                time_label = f"(Ring: {ring_time_str}, Max Wait: {max_wait_str})"
                # Haal leden op
        ...
        # Check Ring Groups (als geen queue)
        if node_type == "Unknown" and not ringgroups_df.empty and (type_hint == "RingGroup" or type_hint == "ExtensionNumber" or type_hint == "UnknownType"):
            ...
            if not rg.empty:
                ...
                # Haal ring time op
                ring_time = rg_info.get('Ring time (s)', None)
                ring_time_str = f"{int(ring_time)}s" if pd.notna(ring_time) and isinstance(ring_time,(int,float)) else "N/A"
                time_label = f"(Ring: {ring_time_str})"
                 # Haal leden op
                members = [f"{r_info[col]} ({format_user_details(u_info.iloc[0])})" if not (u_info := users_df[users_df['Naam'] == r_info[col]]).empty else f"{r_info[col]} (❓)" for col in rg_info.index if col.startswith("User ") and pd.notna(r_info:=rg_info)[col]]
                members_str = "\n ".join(members) if members else "(Geen leden)"
...
if all_data:
    ...
    if receptionists_df_primary.empty:
        st.warning("Geen primaire Digital Receptionists gevonden in 'Receptionists.csv'.")
    else:
        for _, dr in receptionists_df_primary.iterrows():
            ...
            with st.expander(f"IVR: {dr_name} ({dr_ext})"):
                ...
                office_check_node = make_node_id("OFFICECHECK", ""); _, office_shape, office_color, _ = get_node_label_and_style("","Check"); create_or_get_node(dot, office_check_node, "Binnen kantooruren?", shape=office_shape, fillcolor=office_color); dot.edge(start_node_id, office_check_node)
                ...
                # Helper om bestemmingen te tekenen (inclusief No Answer)
                def draw_destination(source_node_id, edge_label, dest_string):
                    dest_type, dest_id = parse_destination(dest_string)
                    target_node_id = make_node_id("END", f"{source_node_id}_{edge_label.replace(' ','_')}")
                    target_label, target_shape, target_color, target_node_type = get_node_label_and_style(None, "EndCall", all_data)
                    if dest_type:
                        target_node_id = make_node_id(f"DEST_{dest_type}", f"{source_node_id}_{edge_label.replace(' ','_')}_{dest_id}")
                        target_label, target_shape, target_color, target_node_type = get_node_label_and_style(dest_id, dest_type, all_data)
                    ...
                    if target_node_type in ["Queue", "RingGroup"] and dest_id:
                         df_lookup = queues_df if target_node_type == "Queue" else ringgroups_df
                         ...
                         if not row.empty:
                             ...
                             noans_label, noans_shape, noans_color, _ = get_node_label_and_style(None, "EndCall", all_data)
                             if noans_type:
                                  noans_target_node_id = make_node_id(f"NOANS_{noans_type}", f"{target_node_id}_{noans_id}")
                                  noans_label, noans_shape, noans_color, _ = get_node_label_and_style(noans_id, noans_type, all_data)
                ...
                draw_destination(office_check_node, "Nee", dest_strings['closed'])
                draw_destination(break_check_node, "Ja", dest_strings['break'])
                # Vakantie route (blijft ongewijzigd, teken alleen als ingesteld)
                holiday_type, holiday_id = parse_destination(dest_strings['holiday'])
                if holiday_type: draw_destination(holiday_check_node, "Ja", dest_strings['holiday'])
                else: dot.edge(holiday_check_node, in_hours_node, label="Ja (geen route)") # Link naar volgende stap

                # Menu / Directe Route
                if has_menu:
                    dot.node(in_hours_node, "🎶 Menu speelt...")
                    for key, dest_str in menu_options_strings.items():
                        draw_destination(in_hours_node, f"Kies {key}", dest_str)

                    # *** Timeout pijl met tijd ***
                    timeout_edge_label = f"Timeout{ivr_timeout_label} /\nGeen invoer"
                    draw_destination(in_hours_node, timeout_edge_label, dest_strings['default'])

                    # Invalid (als anders)
                    if dest_strings['invalid'] != dest_strings['default'] and pd.notna(dest_strings['invalid']):
                        draw_destination(in_hours_node, "Invalid", dest_strings['invalid'])
                else: # Geen menu
                    dot.node(in_hours_node, "Geen menu")
                    draw_destination(in_hours_node, "Direct", dest_strings['default'])
